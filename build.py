...
def build(root):
    ...
    feature=next((p for p in posts if p['slug']=='home-viewing-notes'),posts[0] if posts else None)
    featured=f'<a class="feature" href="/posts/{feature["slug"]}/"><span class="kicker">처음 시작한다면</span><h2>{e(feature["title"])}</h2><span class="feature-link">주거 기록부터 시작하기 ↗</span></a>' if feature else ''
    page('/','집을 옮기고 생활을 정리하는 기준',config['description'],f'''<section class="intro"><div class="intro-label">THE HOME LIFE NOTEBOOK <span>HOUSING / MOVING / LIVING COSTS</span></div><h1>집을 옮기는 날도,<br><span>살아가는 매일도.</span></h1><p>집을 살펴보고, 이삿짐을 정리하고, 생활비를 기록하는 일.<br>막막한 준비를 작은 확인 목록으로 나눕니다.</p></section><section class="topics" aria-label="주제별 가이드">{tabs}</section><section class="reading"><div class="feed"><div class="section-title"><h2>최근 발행한 가이드</h2><span>THE LATEST</span></div><div data-feed="latest">{''.join(listing(p) for p in posts[:12]) or '<p>발행된 글이 없습니다.</p>'}</div></div><aside>{featured}<div class="principles"><span class="kicker">OUR APPROACH</span><h2>기억에 맡기지 않고,<br>하나씩 기록합니다.</h2><p>직접 확인한 내용과 남은 질문을 구분하고, 다음에 할 일을 짧고 구체적으로 남깁니다.</p><a href="/about/">작성 원칙 읽기 ↗</a></div></aside></section>''')
    ...
    for p in posts:
        ...
        for i,s in enumerate(p['sections']):
            ...
            if s.get('example'): sections+=f'<div class="example"><div><strong>정리 메모</strong><button type="button" class="copy" aria-label="정리 메모 복사">복사</button></div><pre>{e(s["example"])}</pre><span class="copy-status" role="status"></span></div>'
            sections+='</section>'
        if 'body_html' in p:
            toc, sections = render_body(p)
        related=''.join(f'<a href="/posts/{q["slug"]}/">{e(q["title"])} ↗</a>' for q in posts if q['id']!=p['id'])
        body=f'''<div class="article-layout"><article class="prose"><div class="breadcrumbs"><a href="/">홈</a> / <a href="/category/{p['category']}/">{e(categories[p['category']]['name'])}</a></div><span class="kicker">{e(p['label'])}</span><h1>{e(p['title'])}</h1><p class="lead">{e(p['description'])}</p><div class="byline">{e(config['name'])} · <time datetime="{p['date']}">{p['date']}</time> · {p['minutes']}분 읽기</div><p>{e(p['intro'])}</p>{sections}<div class="article-note">이 글은 생활 기록과 준비 방법을 설명하는 일반 가이드입니다. 계약이나 전문적인 판단이 필요한 사항은 해당 자료와 적절한 전문 안내로 별도 확인하세요.</div><section class="related"><h2>이어 읽기</h2>{related}</section></article><aside class="toc"><strong>이 글의 순서</strong><ol>{toc}</ol><a class="backtop" href="#main">맨 위로 ↑</a></aside></div>'''
        page('/posts/'+p['slug']+'/',p['title'],p['description'],body,p)
    page('/about/','집생활연구소 소개','집생활연구소의 주제와 콘텐츠 작성 원칙을 소개합니다.',f'<article class="prose standalone"><span class="kicker">ABOUT HOME LIFE LAB</span><h1>새로운 공간과 일상,<br>작은 기록부터.</h1><p class="lead">{e(config["name"])}은 주거·이사·생활비를 다루는 한국어 생활 정보 블로그입니다.</p><h2>우리가 다루는 질문</h2><p>집을 둘러본 기록, 이사 준비 목록, 생활비 정리 방법을 소개합니다. 부동산 중개업체나 이사업체가 아닌 독립적인 정보 사이트이며, 개별 계약이나 법률·재무 판단을 대신하지 않습니다.</p><h2>작성과 수정 원칙</h2><p>직접 사용한 후기와 일반 가이드를 구분합니다. 확인하지 않은 경험이나 성능 수치를 만들지 않습니다. 가격과 기능 등 변할 수 있는 정보는 공식 자료와 확인 시점을 함께 검토합니다. 비용 절감이나 계약 결과를 보장하지 않으며, 실제 조건과 확인하지 못한 사항을 구분해서 설명합니다.</p><h2>광고와 제휴</h2><p>협찬 또는 제휴 관계가 있는 글에는 해당 내용을 표시합니다. 현재 사이트에는 광고나 제휴 링크가 연결되어 있지 않습니다.</p><h2>오류 제보</h2><p>수정이 필요한 글은 <a href="/contact/">문의 페이지</a>로 알려주세요.</p></article>')
    page('/contact/','문의','집생활연구소 문의와 콘텐츠 오류 제보 연락처입니다.',f'<article class="prose standalone"><span class="kicker">CONTACT</span><h1>함께 더 정확하게.</h1><p class="lead">오류 제보, 다뤄주길 바라는 주제, 협업 문의를 보내주세요.</p><a class="email" href="mailto:{e(config["email"])}">{e(config["email"])}</a><h2>문의에 포함할 내용</h2><p>관련 글 주소와 확인이 필요한 부분을 함께 적어주세요. 비밀번호, API 키, 회사 기밀 등 민감한 정보는 보내지 마세요.</p><p>이메일 링크는 기기에 설정된 메일 앱을 엽니다. 메일 앱이 없다면 주소를 복사해 이용 중인 이메일 서비스에서 보내주세요.</p></article>')
    page('/privacy/','개인정보처리방침','사이트 이용과 문의 이메일에 관한 데이터 처리 안내입니다.',f'<article class="prose standalone"><span class="kicker">PRIVACY</span><h1>개인정보처리방침</h1><p>적용일: 2026-09-14</p><h2>사이트 기능</h2><p>현재 회원가입, 댓글, 문의 입력 양식은 없습니다. 방문 분석 및 광고 스크립트, 쿠키나 로컬 저장소에 방문자 정보를 기록하는 기능은 포함하지 않았습니다. 정리 메모 복사는 사용자가 버튼을 누를 때 해당 문장을 기기의 클립보드에 복사하며, 서버로 보내지 않습니다.</p><h2>호스팅과 접속 정보</h2><p>Cloudflare를 통한 페이지 전달과 보안 과정에서 IP 주소와 요청 정보 등이 처리될 수 있습니다. 제공자의 처리 내용은 <a href="https://www.cloudflare.com/privacypolicy/">Cloudflare 개인정보 안내</a>를 참고하세요.</p><h2>이메일 문의</h2><p>운영자는 회신 주소와 문의 내용을 문의 응대에 필요한 범위에서 사용합니다. 목적이 끝나면 불필요한 정보를 삭제합니다. 이메일 서비스의 처리 조건도 함께 적용될 수 있습니다.</p><h2>변경과 연락처</h2><p>광고나 분석 도구 도입 등 실제 처리 방식이 바뀌면 이 안내와 적용일을 갱신합니다. 문의 정보 삭제 및 처리 관련 요청은 <a href="mailto:{e(config["email"])}">{e(config["email"])}</a>로 보내주세요.</p></article>')
    page('/404.html','페이지를 찾을 수 없습니다','주소를 확인하거나 집생활연구소 홈에서 가이드를 찾아보세요.','<section class="page-heading error"><span class="kicker">404 / NOT FOUND</span><h1>잠깐, 경로를<br>다시 확인해주세요.</h1><p>페이지가 이동했거나 주소가 잘못되었을 수 있습니다.</p><a class="button" href="/">가이드 홈으로</a></section>')
