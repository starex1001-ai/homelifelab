# 집생활연구소 · GitHub 업로드용 사이트

주거·이사·생활비 분야의 정적 블로그입니다. 공개 주소는 `https://homelifelab.webnote-lab.workers.dev`로 설정되어 있습니다. 실제 Worker 생성과 이 주소의 배포 여부는 별도로 확인해야 합니다. 테크픽랩 저장소를 덮어쓰지 말고 집생활연구소용 저장소에 올리세요.

## 가장 먼저 할 일

1. ZIP을 풀고 모든 내용을 GitHub의 집생활연구소 저장소 최상위에 업로드합니다. `dist`, `content`, `public`, `build.py`, `site.json`, `wrangler.jsonc`가 최상위에 있어야 합니다.
2. Cloudflare에서 해당 저장소를 `homelifelab` Worker에 연결합니다.
...
Google 인증 HTML을 받으면 `public/` 최상위에 넣고 다시 빌드하세요. `dist`에만 넣으면 다음 빌드에서 사라집니다. 테크픽랩 인증 파일은 집생활연구소에 재사용하지 않았습니다.

## 디자인 및 변경 사항

테크픽랩의 3열 최신글 카드와 짙은 네이비 히어로를 복제하지 않았습니다. 파란색 상단 안내 영역, 가로 주제 탐색, 분류와 본문을 나눈 글 목록, 하단 시작 가이드, 데스크톱 본문 옆 고정 목차로 구성했습니다. 모바일에서는 단일 열로 전환합니다.

샘플 글 3편은 주거 관찰 기록·이삿짐 상자 목록·생활비 분류을 다루는 일반 가이드입니다. 특정 제품의 가상 사용 후기나 검증되지 않은 성능 수치는 포함하지 않았습니다. 글 안의 정리 메모를 복사할 수 있고 실패 시 수동 복사 안내가 나옵니다.
